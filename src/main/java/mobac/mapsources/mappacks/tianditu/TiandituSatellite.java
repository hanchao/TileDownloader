 ...
 */
public class TiandituSatellite extends AbstractHttpMapSource {

	private static String[] SERVERS = { "t1", "t2", "t3", "t4", "t5", "t6", "t7" };
	private static int SERVER_NUM = 0;
	
	public TiandituSatellite() {
		super("TiandituSatellite", 3, 18, TileImageType.JPG, TileUpdate.None);
	}

	@Override
	public String toString() {
		return "Tianditu Satellite";
	}

	public String getTileUrl(int zoom, int tilex, int tiley) {
		String server = SERVERS[SERVER_NUM];
		SERVER_NUM = (SERVER_NUM + 1) % SERVERS.length;
		return "http://" + server + ".tianditu.cn/DataServer?T=img_w&X=" + tilex + "&Y=" + tiley + "&L=" + zoom;
	}
